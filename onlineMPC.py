...
class onlineMCP(object):
    ...
    def generate_new_request(self):
        for port in self.occupied_port:
            ...
            if eta<self.retrieving_probability and len(self.parked_agents)!=0:
                ...
                k=np.random.randint(num_p)
                ak=self.unused_agents[k]
                assert(ak.loc==OUTVERTEX)
                ak.loc=port
                self.parking_agents.append(ak)
    ...
    def init_system(self):
        ...
        # unused agents
        self.unused_agents=[Agent(OUTVERTEX,None,i) for i in range(len(self.parking_spots))]
        # vertices and visiting orders
        self.vertices=[Vertex(x,y) for x in range(self.xmax) for y in range(self.ymax)]
