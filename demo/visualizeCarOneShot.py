...
class Animation:
    ...
    def create_car(self,ax,curr):
        ...
        imobj = ax.imshow(img,extent=[curr[0]-0.5, curr[0]+0.5,curr[1]-0.5, curr[1]+0.5],alpha=1)
        return imobj
    ...
    def __init__(self,system_info, sol_info):
        ...
        for i in range(0,self.num_robots):
            ...
            if i<self.num_retrieval:
                self.selected_rectangles[i]=Rectangle((starti[0]-0.5, starti[1]-0.5), 1,1, facecolor='none', edgecolor='red',linewidth=3)
                self.patches.append(self.selected_rectangles[i])   
            #name = d["name"]
            name=str(i)
    ...
    def animate_func(self, i):
        for k in range(0,len(self.paths)):
            ...
            if k<self.num_retrieval:
                self.selected_rectangles[k].set_xy((p[0]-0.5,p[1]-0.5))
